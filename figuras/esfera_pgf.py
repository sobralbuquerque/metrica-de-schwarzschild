...
scalefactor = 1  #1 para qualidade normal, 4 para compilação rápida
...
cos = np.cos
sin = np.sin
phi, theta = np.mgrid[0.0:pi:(scalefactor**(-1))*40j, 0.0:2.0*pi:(scalefactor**(-1))*40j]
...
#Renderizar
ax.plot_surface(
    x, y, z,  rstride=1, cstride=1, color='c', alpha=0.15, linewidth=0) #esfera

ax.scatter(xx, yy, zz, color="darkblue", s=20, zorder=10)  # pontos

plt.plot(x_1, y_1, z_1, color="darkblue", alpha=0.8, zorder=0)
plt.plot(x_2, y_2, z_2, color="darkblue", alpha=0.8, zorder=0)
plt.plot(x_3, y_3, z_3, color="darkblue", alpha=0.2, zorder=0, ls='--')
...
base_label = [r'$\hat{e}_{\phi}$', r'$\hat{e}_{\theta}$', r'$\boldsymbol{\lambda}$']
...
for v in base:
    ...
    a = Arrow3D([P_0[0], P_0[0]+v[0]], [P_0[1], P_0[1]+v[1]],
                ...
                lw=1.5, arrowstyle="-|>", color="k",zorder=8)
    ax.add_artist(a)
